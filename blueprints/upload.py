...
from Upload import get_paths, is_unique, is_allowed
import uuid
import tasks
...
def upload():
    ...
    try:
        ...
        upload_files[:] = [file for file in upload_files if is_allowed(file.filename)]
        ...
        display_folder = current_app.config['DISPLAY_FOLDER']

        upload_progress['status'] = f'upload location at {process_folder}...'
        socketio.emit('progress_update', upload_progress)
        ...
        socketio.start_background_task(tasks.start_upload, current_app._get_current_object(), upload_progress, task_id, N_upload_files, process_folder, display_folder, file_paths)
